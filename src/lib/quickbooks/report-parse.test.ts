...
// Shaped to match the real live QBO ProfitAndLossDetail response (confirmed
// against the Intuit sandbox): no row ever carries `group`; everything sits
// one level under an unnamed "Ordinary Income/Expenses" / "Other
// Income/Expense" wrapper, and a bare, headerless "Net Income" total can
// trail as its own top-level sibling.
function detailFixture(opts?: { incomeSummaryAmount?: string }) {
  const incomeSummaryAmount = opts?.incomeSummaryAmount ?? "4000.00";
  return {
      ...
      Row: [
        {
            ...
            Row: [
              {
                ...
                Summary: {
                  ColData: PNL_DETAIL_COLUMNS.map((k) =>
                    k === "subt_nat_amount" ? { value: incomeSummaryAmount } : { value: "" },
                  ),
                },
              },
              {
          ...
          Summary: {
            ColData: PNL_DETAIL_COLUMNS.map((k) =>
              k === "subt_nat_amount" ? { value: "3830.00" } : { value: "" },
            ),
          },
        },
        {
          type: "Section",
          Header: { ColData: [{ value: "Other Income/Expense" }] },
          Rows: {
            Row: [
              {
                type: "Section",
        ...
        // a headerless, childless trailing grand-total row — must be skipped
        {
          type: "Section",
...
test("parseProfitAndLossDetail throws when a section's lines don't tie to its Summary", () => {
  // claim the Income Summary is 9999.00 vs the real 4000.00
  const j = detailFixture({ incomeSummaryAmount: "9999.00" });
